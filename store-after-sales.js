(function(){
  ...
  viewOrderDetail = async function(orderNo,orderDate,fromReport=false){
    ...
    document.getElementById('list').innerHTML=`<div class="big-store-title">${currentStore?esc(currentStore.name):'客户账单'}</div><div class="amount-summary-banner"><span style="font-size:16px;color:var(--primary);"><strong>实收：${money(sum)}</strong>${afterBadge}</span></div><div class="detail-action-row"><button class="delivery-note-btn delivery-note-btn-primary detail-delivery-action" type="button" onclick="generateDeliveryNote('${esc(orderNo)}','${orderDate}')">生成单据</button><div class="detail-secondary-actions"><button class="smallbtn" style="background:#fdf6ec;color:#e6a23c;" onclick="editExistingOrder('${esc(orderNo)}','${orderDate}','${raw}')">✏️ 修改订单</button><button class="smallbtn" style="background:#fef0f0;color:#f56c6c;border-color:#fde2e2;" onclick="deleteExistingOrder('${esc(orderNo)}','${raw}')">🗑️ 删除订单</button></div></div>${normalRowsHtml}${afterOnlyHtml}`;
  };

  renderHistory = function(){
    STATE='HISTORY';
    const isTemp=String(currentStore.atom).startsWith('NEW_');
    document.getElementById('list').innerHTML=`<div class="big-store-title">${esc(currentStore.name)} ${isTemp?'<span style="font-size:12px;background:#fff1f0;color:#ff4d4f;padding:2px 8px;border-radius:12px;vertical-align:middle;">新门店</span>':''}</div><div style="margin:14px 0;"><button class="btn-new-order" onclick="openOrder('${esc(currentStore.atom)}','${esc(currentStore.name)}')">＋ 新增单据</button></div>${historyOrders.map(o=>{const d=o.created_at?o.created_at.split('T')[0]:'未知日期';const meta=overviewMetaHtml(o.skuCount||0,o.hasAfterSale,'款');return `<div class="history-item" onclick="viewOrderDetail('${esc(o.order_no)}','${d}',false)"><div style="width:100%;display:flex;justify-content:space-between;align-items:center;"><span style="color:var(--primary);font-size:14px;font-weight:bold;">实收：${money(o.saleSum||0)}</span><span style="color:var(--text-muted);font-size:12px;background:#fff;padding:3px 7px;border-radius:999px;border:1px solid var(--line);">${d}</span></div><div style="font-size:13px;color:var(--text-muted);line-height:1.45;margin-top:4px;">${meta}</div><div class="history-item-actions"><span class="history-detail-hint">查看详情</span><button class="delivery-note-btn delivery-note-btn-primary" type="button" onclick="event.stopPropagation(); generateDeliveryNote('${esc(o.order_no)}','${d}')">生成单据</button></div></div>`}).join('')}`;
  };
